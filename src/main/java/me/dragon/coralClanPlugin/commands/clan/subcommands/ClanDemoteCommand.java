...
public class ClanDemoteCommand implements ISubCommand {
	@Override
		...
		if (args.length == 2) {
			...
			AsyncUtils.runAsync(() -> {
				...
				if (targetPlayerBean
					.getRole()
					.equals(Roles.MEMBER)) {
					AsyncUtils.runTask(() -> player.sendMessage("Questo player è già member!"));
					return;
				}

				targetPlayerBean.setRole(Roles.MEMBER);
				...
				player.sendMessage("Player retrocesso a membro!");
			});
		}
	}
}
